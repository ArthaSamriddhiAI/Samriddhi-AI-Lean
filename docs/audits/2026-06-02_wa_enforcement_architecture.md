...
| WA07 | No long dashes anywhere | a + b | HOOK (block long-dash payloads in Write, Edit, `git commit`) + CLAUDE (chat prose) + existing `stripLongDashes` (pipeline output) | Both | Repo | Hooks can scan tool-call payloads but NOT CC free-text, so files and commits get the hook, chat prose stays CLAUDE. See Section 5.4. |
...
| WA25 | Planner chat vs task chats | d | CHAT (project instructions) + the SUBAGENT isolation analogue in CC | Chat | Split | Primarily a chat-surface separation; its CC analogue is planner-thread versus task-subagent isolation. |
...
### 3.6 Stays as-is or chat-only

WA04 (stub, no mechanism). Chat-primary homes that CC tooling cannot reach: WA18, WA25, and the planner-side halves of WA03, WA15, WA23, WA24 (Section 5 and Section 8 cover the chat surface).
...
### 4.2 The plugin tension, surfaced not resolved

The ping asks this be made explicit rather than silently resolved, so here it is plainly.

The codebase-canonical principle says the repo is the single source of truth. A plugin is a distributable bundle that can live in its own repo and drift. Verified (CC 2.1.139), and this is the load-bearing finding: **a Claude Code plugin copies its components' text into the bundle. There is no native mechanism for a plugin-packaged skill to reference an external canonical file in another repo.** Plugins can bundle skills, subagents, commands, hooks, MCP servers, and settings, namespaced as `/plugin-name:skill-name`, but a bundled skill carries its own text. So naive packaging forks the WA prose into the plugin, which is precisely the WA23 failure mode this effort exists to kill.
...
2. **A minimal repo `CLAUDE.md` (about seven lines plus the WA23 pointer) and a `settings.json` `/permissions` set.** Cheap, immediate, every-session benefit. The pointer line operationalizes WA23 on day one; the permission set cuts confirmation round-trips. Keep `CLAUDE.md` ruthlessly short per 5.4.
...
These are genuine judgment calls (WA28 class), surfaced rather than defaulted.
...
3. **WA07 chat-prose gap.** Hooks cannot enforce no-long-dashes on CC's chat replies, only on files and commits. Is the `CLAUDE.md` line plus the existing `stripLongDashes` on pipeline output sufficient for the chat surface, or does the primary want a stronger measure there (there is no clean mechanical one in current CC)? Accept the residual gap, or escalate.
...
6. **Chat-surface homes.** WA17, WA18, WA23, WA24, WA25 (and the planner halves of WA03, WA15) need a project-instruction home on the web or desktop chat surface, which has no hooks or subagents. The recommendation is a short "Samriddhi conventions" project-instruction block that points at the repo WA files (the same pointer-not-copy discipline). Confirm the primary wants the chat-side block authored as part of the build, and where its canonical text lives (it should still point at `docs/working_agreements/`, but the chat may not have the repo mounted, which loops back to question 1).
