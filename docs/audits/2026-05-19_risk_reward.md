...
## 10. Step 3a execution note: Probe 4 refactor (regime narrative)

Step 3a (ADR-0014, Option A) regenerates fund `monthly_nav` as index co-movement. This relocated the rate-cut regime beat: it was hand-placed in gilt-fund `monthly_nav` (+4.5% Nov->Dec 2026), now it lives at the gilt index (`crisil_dynamic_gilt`, `nifty_10y_gsec`, both verified at +4.5%) and propagates to gilt funds beta-scaled (median ~+2.0%). `scripts/_verify-snapshot-enrichment.ts` Probe 4 was refactored mid-workstream to reflect ADR-0014's index-source-of-truth methodology: it now asserts the beat at the index (source) and the beta-scaled propagation at the funds (destination). This is an architectural improvement (single source of truth), not a degraded test; the pattern is established for future regime-narrative additions (place the beat at the index; fund response derives by construction). Probes 1, 2, 3, 5 (stock `monthly_prices`) were unaffected. The regenerated snapshots and the Probe 4 rewrite were committed atomically so the deterministic regression contract is green at every point in git history.

## 11. Step 5 observation: LLM-fallback routing across the S2 seed

All 6 S2 fixtures trigger the LLM-fallback rollup path. Breakdown: 5 of 6 via structural-gap rules (portfolio evaluable <= 40% for bhatt/iyengar/menon/sharma; sentinelled-sleeve > 35% for malhotra), 1 of 6 via the underperformance rule (surana, portfolio IR -0.77). This reflects the demo seed's deliberate complexity-heavy curation (substantial PMS, AIF, FD, tax-free bond, and savings allocations are the norm at this HNI wealth tier); the trigger logic is calibrated correctly for these case types. A future demo seed including cleaner MF-heavy outperforming archetypes would route those to the templated path. The templated path is independently covered by the `_verify-risk-reward.ts` probes (19/19 passing). This routing breakdown is useful for the model-portfolio workstream (which will want to know which cases route through which paths when designing its overrides) and for future demo-seed extensions.

## 12. Step 6 disclosure: systematic-negative information ratio is a methodology consequence
