...
**Numbering conventions for debt log entries.** Prompts that specify exact P/D/T numbers may collide with existing log state, because chat context does not always reflect the current repo state of the logs (the D1-to-D6 renumbering this round surfaced this). Confirm the next available number against the current log before adding an entry; future prompts should say "the next available P-series number" rather than hard-coding one.

**Working agreement 11 (dual-write hand-off).** Every hand-off note is dual-written: once here in the workstream's audit file (the per-workstream record) and once in `docs/workstreams/` as a workstream-scoped hand-off doc (the cross-workstream durable record), using the existing `<workstream>_<purpose>.md` naming pattern. The dual write is itself a verification mechanism: if the orchestration chat and Claude Code drift, two files have to stay aligned, which makes the gap visible. This applies to this workstream retroactively (the A2 hand-off content is mirrored to `docs/workstreams/a2_classification_handoff.md`) and to all four remaining capability workstreams going forward.

**Discipline demonstrated: the Layer 1 / Layer 2 split made an investigation answerable.** Step 5 review flagged a recurring "5Y return 6.86%" across three funds that looked like Layer 2 fabrication. Because verdicts are deterministic Layer 1 and only the prose is Layer 2, the question "is Layer 2 inventing this?" had a clean place to stand: the figure was traced to the source snapshot and confirmed as genuine coincidence (two funds whose 5Y returns both round to 6.86%). The split did not just prevent the failure mode; it made the failure mode investigable, and the investigation surfaced a real general question (Layer 2 fact-grounding, logged as T9) that improves the system without forcing an immediate fix. The hand-off record captures how the discipline produced value, not only what shipped.
