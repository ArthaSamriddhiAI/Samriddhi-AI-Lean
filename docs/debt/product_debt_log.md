...
| P17 | Risk-free-rate configurability. RF is the documented 5.25% repo rate (ADR-0012), sourced as a named constant in `risk-reward-stats.ts` (not from snapshot provenance, which carries no rf field; D2). Multi-RF evaluation (for example a 91-day T-bill baseline) is deferred until the model-portfolio workstream has a concrete need. PARTIALLY DELIVERED 2026-06-02 (T-5.14): the time-varying `seriesRiskFree` socket is wired (`ca515bb`) and the real 91-day T-Bill yield series is stored in real t0 (`debt_yield_primitives.tbill_91d_yield`) and demonstrated in the re-fire; the shipped basis stays the constant 0.0525 so per-holding and sleeve agree on R_f (beta is R_f-invariant), so promoting the time-varying series to the shipped basis is now a config flip, not new code. | Low | Risk-reward (D2) | Model-portfolio workstream |
...
| P23 | Live S2 pipeline does not generate LLM rollups. The live `runRiskRewardDeterministic` produces templated rollups only; the 6 backfilled S2 fixtures carried LLM-generated rollups from the Step 5 backfill. This is a deliberate cost-control choice. **Updated 2026-06-09 (render bundle audit, T-5.09):** after the PR #14 real-t0 re-fire, the five canonical Samriddhi 2 fixtures now carry `templated` rollups (`risk_reward_stats.rollup.generation_method: templated`), not LLM-fallback ones (the sixth, sharma-s2, is archived per P42); the live-LLM-rollup question stands, but the persisted fixtures are now uniformly deterministic, which is what lets the render bundle hold zero-spend. A future workstream desiring live LLM rollups must wire `runRiskRewardStats` (the LLM-capable orchestrator) into the live pipeline with proper WA12 handling at the runtime layer. Cross-references WA12, ADR-0020. | Low | Risk-reward (architectural audit) | Future workstream wiring live LLM rollups |
...
| P48 | Enterprise async-QA layer for A1 over Samriddhi 2 diagnostics. A1 (the institutional contrarian) as a periodic quality audit over already-generated Samriddhi 2 diagnostic cases, not an inline case-generation-runtime agent. It would read produced diagnostics, interrogate the believability of their own evidence and metrics (implausibly clean alpha/beta, benchmark aptness, unpriced scenarios, the real gap A3 cannot fill because A3 reasons forward from the numbers, not at them), learn from how advisors iterate on those cases, and feed improvements back into the product indirectly (prompt / threshold / methodology updates) rather than as a live in-case voice. This is the agentic-QA-loop direction: an autonomous LLM agent running its own QA pass, which is the heavier class of complexity that must be earned through demonstrated need, not assumed (ADR-0040's governing principle; ADR-0038 precedent). Contingent on enterprise deployment, a corpus of real generated cases, and the agentic-loop infrastructure; it would learn from the human-loop signal tracked in P49. Distinct from P49: P48 is the agentic QA loop, P49 is the human iteration loop it learns from. Parked, not scoped; reshape freely when the trigger nears. The A1 skill-versus-code schema mismatch (T17) is a Samriddhi 1 concern and is not part of this entry. | Deferred | A1 challenge suitability audit (T-5.13); ADR-0040 | Enterprise async-QA / agentic-loop workstream (trigger: enterprise deployment with real-case corpus and advisor-iteration signal) |
| P49 | Human-in-the-loop advisor-iteration capability for Samriddhi 2. The advisor iterates with Samriddhi on a generated diagnostic case and the system aligns to their working style (which framings they keep, which they discard, how they phrase the conversation). This is the human-loop direction: looping with the human advisor, which ADR-0040 records as sound and welcome, explicitly distinct from looping with an autonomous LLM agent that decides for itself. It is the prerequisite signal the A1 async-QA layer (P48) would learn from: without a record of how advisors refine cases, an autonomous QA pass has nothing to align to. Parked, not scoped; the human-loop and the agentic-loop are different capabilities and the distinction is the point (keep P48 and P49 separate). | Deferred | A1 challenge suitability audit (T-5.13); ADR-0040 | Advisor-iteration / human-in-the-loop workstream (enterprise direction) |
| P50 | Section 06 continuous gross-and-net performance line deferred to the data-management workstream. The v7.2 wireframe's centerpiece performance chart (gross market value versus net invested cost over time) cannot be drawn from reconciled data today: the eCAS transaction NAV basis and the snapshot `monthly_nav` are inconsistent universes (see `docs/audits/2026-06-09_section06_performance_line_data.md` and its addendum). Root cause: `generate_ecas.py`'s `simulate_history` synthesizes NAVs from an assumed CAGR (backwards from current value at current NAV), not from the snapshot `monthly_nav`; the eCAS and the snapshot share only the final market value, so a coherent line covering more than the mutual funds (46 to 52%) does not reconcile against the 70% honesty floor. Fix (data-management workstream): re-anchor `simulate_history` to the snapshot `monthly_nav`/`monthly_prices` so eCAS units and snapshot NAVs share one basis; resolve the fund-name mismatches against the real snapshot universe (Kotak Emerging Equity, ICICI Balanced Advantage, HDFC Index Nifty 50 did not cleanly exist and were fuzzy-matched); regenerate the eCAS set; then the line clears 70% honestly. Source artifacts: the four generators in the data repo `ArthaSamriddhiAI/Samriddhi-AI-Data-Snapshots` at `generators/` (`generate_snapshots.py`, `generate_ecas.py`, `generate_a1_a5_ingestion.py`, `generate_altformat.py`), committed as provenance with original hardcoded paths (rehome and rewire first); governed by that repo's ADR-0002; the limitation is the first item in that repo's `generators/METHODOLOGY.md`. Section 06 ships the window bars meanwhile (WA16: no line from data that does not reconcile). | Medium | Render bundle v7.2 rebuild (section 06 reconciliation) | Data-management workstream (eCAS NAV-basis reconciliation) |
| P51 | `lib/agents/structured-holdings.ts` drops the per-holding cost-basis and purchase-date fields when it transforms raw holdings into `StructuredHoldings`. The raw `holdings_a1_a5.json` rows carry `cost_basis_total_inr`, `cost_basis_per_unit_inr`, `purchase_date`, and `quantity` on all 41 holdings, but the structured shape keeps only what the current agents need, so a future section 06 performance-line build (P50) that wants cost basis at the holdings layer would have to re-thread these fields through the transform. Low urgency while the line is deferred; fold into the section 06 line build. | Low | Render bundle v7.2 rebuild (section 06 data audit) | Data-management workstream (section 06 line build, P50) |
| P52 | v7.2 wireframe sections not rendered in the composed page (ADR-0047). Section 05 (critical review) is omitted per ADR-0040 (the A1 collision) and is recorded here for completeness, not as open work; it would only return if the A1 async-QA layer lands (P48). Sections 10 (valuation) and 12 (decision history) are deferred: the composed page renders 01 to 04, 06 to 09, and 11, leaving 10 and 12 for a later fidelity pass when their data surfaces are ready. Neither blocks the render bundle; tracked so the gap stays visible. | Low | Render bundle v7.2 rebuild | Later v7.2 fidelity pass (10, 12); ADR-0040 (05); P48 (A1) |
| P53 | Dead `.ar-c-*` accordion CSS to sweep. The Concept C severity-accordion skeleton (superseded by the v7.2 composed page, ADR-0047) left `.ar-c-*` style rules in `app/globals.css` that the composed page no longer references. Harmless unused selectors, but they should be swept so the stylesheet matches the shipped structure. Deferred from this pass to keep the PR single-purpose (render fidelity, not a CSS-churn cleanup); a small follow-up removes them after confirming no remaining references. | Low | Render bundle v7.2 rebuild (ADR-0047 supersede) | Small CSS-cleanup follow-up |
