...
## Amendment (2026-06-10, Package 07): the WA12 gate proves no-spend structurally

The WA12 backstop in `scripts/hooks/gate-sensitive-bash.sh` previously fired on every runner invocation matching its keywords (refire, pipeline, backfill), including deterministic fixture scripts with no SDK reach; the agent then certified its own innocence and self-cleared the one-shot marker, which is structurally the thing WA12 exists to keep a human in the loop on. The gate now auto-allows an invocation only when `scripts/hooks/check-sdk-reach.mjs` PROVES every script file named in the command has no Anthropic SDK reach through its static import chain (value imports, require, dynamic import, and value re-exports are walked; type-only imports are erased at runtime and skipped; an anthropic-matching specifier is reach). Everything unproven fails closed into the existing marker protocol: no extractable script path (including `npm run` aliases, now matched), unresolvable imports, or genuine reach. The proof is the import graph, not an assertion. Verified by `scripts/hooks/_verify-wa12-gate.sh` (no-fire on proven no-spend; still fires on direct, transitive, and unprovable cases; marker one-shot semantics intact). Disposition: amends ADR-0044 in place.
