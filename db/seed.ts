...
/* CaseFixture is the on-disk shape of db/fixtures/cases/*.json. Mirrors
 * the Case row 1:1, with contentJson + tokenUsageJson stored as nested
 * objects rather than escaped strings for readability. Re-stringified
 * on seed.
 *
 * Cases generated by the real Samriddhi reasoning pipeline cost real API
 * spend (~Rs 250-500 per case at Sonnet 4.6 pricing). They are persisted
 * as fixtures so demo state is reliable and zero-cost on db:seed. The
 * post-funding five-case batch (commit 20 deferred per DEFERRED.md) and
 * Slice 3's Sharma case will follow this same pattern. */
type CaseFixture = {
  id: string;
  investorId: string;
  snapshotId: string;
...
export async function seedDatabase() {
  ...
  const caseFixtures = await loadCaseFixtures();
  for (const fixture of caseFixtures) {
    const record = {
      id: fixture.id,
      investorId: fixture.investorId,
      snapshotId: fixture.snapshotId,
      workflow: fixture.workflow,
      severity: fixture.severity,
      headline: fixture.headline,
      status: fixture.status,
      frozenAt: new Date(fixture.frozenAt),
      contentJson: JSON.stringify(fixture.content),
  ...
  return { snapshots: SNAPSHOTS.length, investors: INVESTORS.length, cases: caseFixtures.length };
}

async function loadCaseFixtures(): Promise<CaseFixture[]> {
  let entries: string[];
  try {
    entries = await fs.readdir(CASE_FIXTURES_DIR);
  } catch (err) {
  ...
  for (const name of entries) {
    ...
    const filePath = path.join(CASE_FIXTURES_DIR, name);
    const raw = await fs.readFile(filePath, "utf-8");
    out.push(JSON.parse(raw) as CaseFixture);
  }
  return out;
}
