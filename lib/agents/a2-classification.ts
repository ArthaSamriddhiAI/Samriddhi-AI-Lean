...
function buildReasonPrompt(
  ...
  return [
    ...
    "```",
    ``,
    `## reasoning_summary discipline`,
    ``,
    `When every holding is Maintain (no drivers anywhere), do not call the`,
    `portfolio healthy or clean. A2's column being silent does not mean the`,
    `portfolio is healthy; it means A2 has no per-holding conversation to`,
    `flag in this case. Any material observation on the portfolio (cash`,
    `drag, allocation drift, deployment efficiency) lives at the S1`,
    `portfolio surface per the propagation rules. State that A2 has nothing`,
    `to flag per-holding and that portfolio-level observations, if any, sit`,
    `with S1; do not characterise the portfolio's overall health, which is`,
    `S1's responsibility, not A2's.`,
    ``,
    `When holdings reach Review or Discuss through different driver types`,
    `(position vs thesis vs wrapper vs sector vs complexity_premium vs`,
    `regulatory), name the driver type per holding rather than collapsing`,
    `distinct mechanisms into one phrase. For example, do not say several`,
    `holdings are "crossing the 15% escalate threshold" when one is at`,
    `Review on a broken thesis and another on a position breach; attribute`,
    `each holding to its actual driver.`,
    ``,
    `## Output`,
    ``,
