...
function buildPrompt(input: S1Input): string {
  return [
    ...
    `the time_series_performance block (return evidence: trailing-window and`,
    `benchmark-relative returns plus cross-snapshot evolution; cite snapshot IDs,`,
    `never invent figures, surface sentinels honestly), and the router decision`,
    `are below. The pre_observations array points at`,
