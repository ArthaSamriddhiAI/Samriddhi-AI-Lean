...
Computed by Layer 1 (you cite, never alter): for each trimmed or exited position, its current weight, the breach threshold it crossed, the target weight (10%, the foundation section 3 single-position flag threshold, for a trim; 0 for an exit), the total trim in percentage points, and the glide-path steps (step number, weight points trimmed, resulting weight, and the weight level at which the advisor takes it). Plus the redeployment: the funding (capital freed by trims and exits, and any deployable cash above its target), the under-target sleeves it flows to (each filled toward its TARGET, the band is cushion, not a destination) and by how much, and any leftover that honestly remains in cash.

This is one gap-closing operation: move the allocation toward the target. Trimming an overweight and deploying idle cash are the same operation at different points on a spectrum (an established drifter with little cash versus a new investor sitting in cash). Cash is dry powder, not a position: existing cash above its target funds the under-target side of the gap, and a heavy cash balance is undeployed capital to deploy, not a concentration to trim. Capital that exceeds the total gap-to-target is honest leftover that stays in cash; surface it as advisor guidance (available for staged deployment, or grounds to revisit the target weights), never hidden by over-filling a sleeve past its target, never apologised for to the client.

A3 owns the glide-path cadence (how many steps, how large each step), an execution-pacing choice to manage market impact and tax events, not a concentration threshold. A3 invents no concentration thresholds (the 10% flag and 15% escalate come from the foundation, read through M0) and no allocation targets (the per-investor sleeve targets and bands come from the investor's mandate, read through M0, falling back to the foundation model bands when no mandate is set). The redeployment numbers close: capital freed by trims and exits plus deployed cash equals the redeployment plus the leftover to cash.
